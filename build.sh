...
function github-runner() {
    ...
    tar xzf ./actions-runner-linux-arm64-${build_version}.tar.gz && rm -rf actions-runner-linux-arm64-2.278.0.tar.gz
    sed -i '3,9d' ./config.sh
    sed -i '3,8d' ./run.sh
    tar -zcvf actions-runner-linux-arm64-2.304.0.tar.gz *
