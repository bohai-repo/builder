...
function frpc() {
    ...
    mkdir build frpc_${build_version} && cd build
    ...
    mv frp_${build_version}_linux_${PLATFORM}/frpc frpc_${build_version}/ && tar zcf frpc_${build_version}.tar.gz frpc_${build_version}
    rm -rf frpc_${build_version} && cd ../
    mv build/frpc_${build_version}.tar.gz ./
}
